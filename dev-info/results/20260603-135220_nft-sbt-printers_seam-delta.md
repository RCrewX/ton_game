...
> truth — it is **regenerated by the single core deploy** (`pnpm build --all && pnpm
> deploy:testnet`) before uap reads it. The printers are now part of that one deploy: it
> writes their addresses + code hashes + opcodes and refreshes the (already-stale) sbtn BOCs.
...
1. **Block on** the `deployment_latest.json` produced by the next `pnpm deploy:testnet`
   (printer addresses + code hashes + opcodes + refreshed sbtn BOCs).
