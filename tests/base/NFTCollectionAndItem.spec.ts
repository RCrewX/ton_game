...
import { NFTCollection } from '../../wrappers/tep/nft/NFTCollection';
...
describe('02_nft', () => {
    let GAS_LOG = new GasLogAndSave('02_nft');
    ...
    it('should increment item index if last', async () => {
        ...
        expect(newNextItemIndex).toBe(nextItemIndex + 1n);
    });
    ...
    it('should deploy some nfts via batch deploy', async () => {
        ...
        const content = beginCell().storeStringTail('Content').endCell();
        const indexes = [nextItemIndex, nextItemIndex + 1n, nextItemIndex + 2n];
    ...
    it.skip('[bench] should deploy 100 nfts via batch deploy', async () => {
    ...
    it('should not change admin from non-admin', async () => {
        const notAdmin = await blockchain.treasury('not-admin');
        const newOwner = await blockchain.treasury('new-owner');

        const result = await nftCollection.sendChangeOwner(notAdmin.getSender(), {
            value: toNano('0.05'),
            newOwner: newOwner.address,
        });
    ...
    it('should return get_nft_content for index with individual content', async () => {
        const content = beginCell().storeStringTail('ItemContent').endCell();
        const { nextItemIndex } = await nftCollection.getCollectionData();
        await nftCollection.sendDeployNft(owner.getSender(), {
            to: owner.address,
            index: nextItemIndex,
        ...
        const fullContent = await nftCollection.getNftContent(nextItemIndex, content);
        expect(fullContent).toBeDefined();
        expect(fullContent instanceof Cell).toBe(true);
    });

    it('should return royalty_params with edge values (0/1)', async () => {
        const zeroRoyaltyCollection = blockchain.openContract(
            NFTCollection.createFromConfig(
                {
                    nftItemCode,
                    ownerAddress: owner.address,
        ...
        await zeroRoyaltyCollection.sendDeploy(owner.getSender(), toNano('0.05'));
        const params = await zeroRoyaltyCollection.getRoyaltyParams();
        expect(params.numerator).toBe(0);
        expect(params.denominator).toBe(1);
        expect(params.royaltyAddress).toEqualAddress(owner.address);
    });
    });
    ...
    it.skip('should fail when response address in different workchain', async () => {
        const workchain = -1;
        const nftOwner = await blockchain.treasury('nft-owner');
        const nftReceiverAddress = randomAddress();
